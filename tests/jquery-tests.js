//todo: get jquery spec passing against bean 100%
// all uncommented spec currently should be passing in all major browsers.

sink('add', function (test, ok, before, after) {

  test("null or undefined handler", 2, function() {
   ...
   try {
     $("#firstp").click(undefined);
     ok(true, "Passing an undefined handler will not throw an exception");
   } catch (e) {}
  });

  test("bind(), with data", 3, function() {
   ...
   var handler2 = function(event) {
     ok( event.data == test, "bind() with function data, Check value of passed data" );
   };
   $("#firstp").bind("click", test, handler2).click().unbind("click", handler2);
  });

  test("click(), with data", 2, function() {
   var handler = function(event) {
     ok( event.data, "bind() with data, check passed data exists" );
     ok( event.data.foo == "bar", "bind() with data, Check value of passed data" );
   };
   var x = $("#firstp").click({foo: "bar"}, handler).click().unbind("click", handler);
  });

  test("bind(), with data, trigger with data", 4, function() {
  ...
  test("bind(), multiple events at once", 2, function() {
   var clickCounter = 0,
     mouseoverCounter = 0;
   var handler = function(event) {
     if (event.type == "click")
  ...
  test("bind(), five events at once", 1, function() {
   var count = 0,
        handler = function(event) {
         count++;
        };

   $("#firstp").bind("click mouseover foo bar baz", handler)
      .trigger("click").trigger("mouseover")
        .trigger("foo").trigger("bar")
         .trigger("baz");

    ok( count === 5, "bind() five events at once" );
  });

  test("bind(), multiple events at once and namespaces", 7, function() {
   var cur, obj = {};

   var div = $("<div/>").bind("focusin.a", function(e) {
     ok( e.type == cur, "Verify right single event was fired." );
   });
   ...
   div = $("<div/>").bind("click mouseover", obj, function(e) {
     ok( e.type == cur, "Verify right multi event was fired." );
     ok( e.data == obj, "Make sure the data came in correctly." );
   });
   ...
   div = $("<div/>").bind("focusin.a focusout.b", function(e) {
     ok( e.type == cur, "Verify right multi event was fired." );
   });
  ...
  // test("bind(), namespace with special add", 24, function() {
  //
  //  var div = $("<div/>").bind("test", function(e) {
  ...
  test("bind(), no data", 1, function() {
   ...
   $("#firstp").bind("click", handler).trigger("click").remove();
  });

  test("bind/one/unbind(Object)", 6, function(){
    ...
    ok( clickCounter == 3, "bind(Object)"  + ' ' + clickCounter);
    ok( mouseoverCounter == 3, "bind(Object)" );

    trigger();

    ok( clickCounter == 4, "bind(Object)" );
    ok( mouseoverCounter == 4, "bind(Object)" );
...
// });
  ...
  // test("live/delegate immediate propagation", 2, function() {
  //  var $p = $("#firstp"), $a = $p.find("a:first"), lastClick;
  //
  //  lastClick = "";
